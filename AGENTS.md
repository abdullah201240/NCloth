# NCloth Fashion Admin Permanent Design, Security & Architecture Rules
...
- **NO DELETE OPERATIONS:** Never provide hard "Delete" buttons, delete actions, or SQL `DELETE` calls in any part of the system (categories, subcategories, products, variants, orders, customers, media).
...
- **Input Sanitization & Constraints:** Explicitly enforce string trimming (`.trim()`), length bounds (`.min()`, `.max()`), regex patterns for SKUs/barcodes/slugs, and reject malicious payloads (`.strict()`).
